...
public sealed class NetDocumentsSyncServiceExportTests
{
    ...
    [Fact]
    public async Task EnumerateContainerDocumentsAsync_ReadsKnownVersionCountFromVersionsLiteObject()
    {
        var tempRoot = Path.Combine(Path.GetTempPath(), $"nd-export-doc-version-count-tests-{Guid.NewGuid():N}");
        Directory.CreateDirectory(tempRoot);
        var dbPath = Path.Combine(tempRoot, "jobstore.db");
        ...
        try
        {
                          ...
                          "standardList": [
                            {
                              "standardAttributes": {
                                "docId": "DOC-300",
                                "description": "Covering Letter saved with ndoffice (1).docx",
                                "sizeBytes": 2048
                              },
                              "versionsLite": {
                                "officialVersionId": "3",
            ...
            var document = Assert.Single(documents);
            Assert.Equal("3", document.OfficialVersionId);
            Assert.Equal(3, document.KnownVersionCount);
            Assert.False(document.HasExactVersionCoverage);
            Assert.Single(document.VersionHints);
        }
        finally
        {
            DeleteIfExists(dbPath);
    ...
    [Fact]
    public async Task EnumerateDocumentVersionsAsync_UsesVersionListWhenAvailable()
    {
        var tempRoot = Path.Combine(Path.GetTempPath(), $"nd-export-version-list-tests-{Guid.NewGuid():N}");
        Directory.CreateDirectory(tempRoot);
        var dbPath = Path.Combine(tempRoot, "jobstore.db");
        ...
        try
        {
            ...
            var handler = new StubHttpHandler(request =>
            {
                ...
                requestPaths.Add(request.RequestUri.PathAndQuery);
                if (string.Equals(request.RequestUri.PathAndQuery, "/v1/Document/DOC-1/versionList", StringComparison.OrdinalIgnoreCase))
                {
                    return JsonResponse(HttpStatusCode.OK, """
                        [
                          { "verNo": 1, "official": false, "sizeBytes": 100, "name": "Agreement.docx" },
                          { "verNo": 2, "official": true, "sizeBytes": 200, "name": "Agreement.docx" }
                        ]
                        """);
                }
            ...
            Assert.Contains("/v1/Document/DOC-1/versionList", requestPaths, StringComparer.OrdinalIgnoreCase);
            Assert.Collection(
                versions,
                version =>
                {
    ...
    [Fact]
    public async Task EnumerateDocumentVersionsAsync_FallsBackToDocInfoGetVersions()
    {
        var tempRoot = Path.Combine(Path.GetTempPath(), $"nd-export-version-docinfo-tests-{Guid.NewGuid():N}");
        Directory.CreateDirectory(tempRoot);
        var dbPath = Path.Combine(tempRoot, "jobstore.db");
        ...
        try
        {
            ...
            var handler = new StubHttpHandler(request =>
            {
                ...
                requestPaths.Add(request.RequestUri.PathAndQuery);
                if (string.Equals(request.RequestUri.PathAndQuery, "/v1/Document/DOC-2?getVersions=true", StringComparison.OrdinalIgnoreCase))
                {
                    return JsonResponse(HttpStatusCode.OK, """
                        {
                          "docVersions": [
                            { "versionId": "4", "official": false, "sizeBytes": 400, "name": "Cover Letter.docx" },
                            { "versionId": "5", "official": true, "sizeBytes": 500, "name": "Cover Letter.docx" }
                          ]
                        }
                        """);
                }
            ...
            var service = CreateSyncService(handler, dbPath);
            var versions = await service.EnumerateDocumentVersionsAsync("DOC-2");

            Assert.Equal(2, versions.Count);
            Assert.Contains("/v1/Document/DOC-2/versionList", requestPaths, StringComparer.OrdinalIgnoreCase);
            Assert.Contains("/v1/Document/DOC-2?getVersions=true", requestPaths, StringComparer.OrdinalIgnoreCase);
            Assert.All(versions, version => Assert.Equal("docInfo", version.DiscoverySource));
        }
        finally
        {
            DeleteIfExists(dbPath);
